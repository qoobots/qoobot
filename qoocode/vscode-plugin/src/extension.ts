...
export function activate(context: vscode.ExtensionContext) {
    console.log('qoocode-vscode: activating...');
    ...
    vscode.window.registerTreeDataProvider('qoocode.projectExplorer', projectExplorer);
    context.subscriptions.push(projectExplorer);

    const skillList = new QooSkillList(context);
    vscode.window.registerTreeDataProvider('qoocode.skillList', skillList);
    context.subscriptions.push(skillList);

    const behaviorTreeProvider = new QooBehaviorTreeProvider(context);
    vscode.window.registerTreeDataProvider('qoocode.behaviorTrees', behaviorTreeProvider);
    ...
    statusBar.command = 'qoocode.doctor';
    statusBar.text = '$(robot-view) qoocode';
    statusBar.tooltip = 'qoocode — Robot Skill Development';
    statusBar.show();
    ...
    console.log('qoocode-vscode: activated');
}
...
function updateProjectContext() {
    const isProject = isQoocodeProject();
    vscode.commands.executeCommand('setContext', 'qoocode.isQoocodeProject', isProject);
}

function isQoocodeProject(): boolean {
