...
| 模块 | 说明 | 状态 |
|------|------|------|
| `mfi/` | Made for QooBot 认证（硬件/软件/安全认证） | ✅ 已完成 |
| `tools/` | 末端执行器生态（夹具、吸盘、焊枪、3D 打印头） | ✅ 已完成 |
| `wearables/` | 可穿戴设备（操作手套、AR 眼镜、外骨骼） | ✅ 已完成 |
| `sensors/` | 外接传感器（触觉皮肤、六维力传感器、激光雷达） | ✅ 已完成 |
| `mobility/` | 移动平台附件（充电桩、升降平台、轨道系统） | ✅ 已完成 |
| `connectivity/` | 配件通信协议（UWB、蓝牙 LE、WiFi Direct、NFC） | ✅ 已完成 |
| `power/` | 配件供电标准（磁吸充电、无线充电、热插拔） | ✅ 已完成 |
| `sdk_gear/` | 配件开发 SDK（驱动模板、测试套件、认证工具） | ✅ 已完成 |
