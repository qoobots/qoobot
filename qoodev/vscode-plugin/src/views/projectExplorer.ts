...
export class QooProjectExplorer implements vscode.TreeDataProvider<QooTreeItem>, vscode.Disposable {
    ...
    private async getRootItems(root: string): Promise<QooTreeItem[]> {
        ...
        // Check if this is a qoodev project
        ...
        if (isProject) {
            // Standard qoodev project layout
            ...
            // Not a qoodev project — show prompt to init
            items.push(new QooTreeItem(
                'Not a qoodev project',
                '',
            ...
            initItem.command = { command: 'qoodev.init', title: 'Init Project' };
            items.push(initItem);
        }
            ...
            if (entry.isDirectory()) {
                ...
                if (type === 'btree') {
                    item.command = {
                        ...
                        arguments: [vscode.Uri.file(entryPath), 'qoodev.behaviorTree']
                    };
                } else {
                    item.command = {
                        command: 'vscode.open',
