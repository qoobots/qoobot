...
// ─────────────────────────────────────────────────────────────────────────────
/**
 * @brief 统一错误码，覆盖编译、运行时、内存、硬件等所有子系统。
 *
 * 错误码分段：
 *   0     — 成功
 *   1xxx  — 通用错误
...
public:
    ...
    struct ErrorInfo {
        ...
        std::string message;
        std::optional<std::string> details; // 可选详细信息（调用栈等）
...
[[nodiscard]] inline Result<T> Ok(T&& v) {
    return Result<T>::ok(std::forward<T>(v));
}
...
// ─────────────────────────────────────────────────────────────────────────────
/**
 * @brief 张量数据类型，对标 ONNX TensorProto 数据类型。
 *
 * 位布局（高 16 位 = 类别，低 16 位 = 精度/通道）：
 *   - 0xxx — 浮点
 *   - 1xxx — 有符号整数
 *   - 2xxx — 无符号整数
 *   - 3xxx — 量化类型
...
/** @brief 返回单个元素占用的字节数，量化类型返回打包后的有效字节数。 */
...
enum class TensorLayout : std::uint8_t {
    /**
     * NCHW：PyTorch 默认，训练友好。
     * 内存排列：[N, C, H, W] — 通道连续。
     */
    NCHW = 0,

    /**
     * NHWC：TensorFlow 默认，GPU 卷积高效。
     * 内存排列：[N, H, W, C] — 像素连续。
     */
    NHWC = 1,
    ...
    /**
     * NCDHW：3D 卷积，[N, C, D, H, W]。
     */
    NCDHW = 3,

    /**
     * NDCHW：Transformer 常用，[N, D, C, H, W]。
     */
    NDCHW = 4,
};
...
// ─────────────────────────────────────────────────────────────────────────────
/**
 * @brief 量化参数，支持逐张量和逐通道量化。
 *
 * 量化公式（对称量化）：
 *   real_value = (int_value - zero_point) * scale
 *
 * 量化公式（非对称量化）：
 *   real_value = int_value * scale + zero_point
 */
struct QuantParams {
    DType target_dtype{DType::QINT8}; ///< 量化后的数据类型
    std::vector<float> scales;          ///< 缩放因子（逐通道时长度 = C）
    std::vector<std::int32_t> zero_points; ///< 零点（通常为 0，对称量化）
    bool per_channel{false};            ///< 是否为逐通道量化
    bool symmetric{true};              ///< 是否为对称量化
